...
class BlockReceiver implements Closeable {
	...
	/**
	 * In the case that the client is writing with a different checksum polynomial
	 * than the block is stored with on disk, the DataNode needs to recalculate
	...
	/**
	 * While writing to mirrorOut, failure to write to mirror should not affect this
	 * datanode unless it is caused by interruption.
	 */
	...
	/**
	 * Translate CRC chunks from the client's checksum implementation to the disk
	 * checksum implementation.
	 * 
	 * This does not verify the original checksums, under the assumption that they
	...
	/**
	 * Check whether checksum needs to be verified. Skip verifying checksum iff this
	 * is not the last one in the pipeline and clientName is non-null. i.e. Checksum
	 * is verified on all the datanodes when the data is being written by a datanode
	 * rather than a client. Whe client is writing the data, protocol includes acks
	 * and only the last datanode needs to verify checksum.
	 * 
	...
	private int receivePacket() throws IOException {
		...
		// TODO 步骤一：blockReceiver接受数据做的第一个事
		...
		// TODO 步骤二：把当前的这个packet发送给下游的节点（数据管道里面的）
		if (mirrorOut != null && !mirrorError) {
			try {
				long begin = Time.monotonicNow();
				//发送给下游
				//因为下游肯定也是启动了blockRecive来接受我们的数据
			...
			try {
				...
				if (onDiskLen < offsetInBlock) {
					...
					if (!shouldNotWriteChecksum && firstByteInBlock % bytesPerChecksum != 0) {
						if (LOG.isDebugEnabled()) {
							LOG.debug("receivePacket for " + block + ": bytesPerChecksum=" + bytesPerChecksum
	...
	/**
	 * Cleanup a partial block if this write is for a replication request (and not
	 * from a client)
	 */
	...
	/**
	 * Adjust the file pointer in the local meta file so that the last checksum will
	 * be overwritten.
	 */
	...
	/**
	 * reads in the partial crc chunk and computes checksum of pre-existing data in
	 * partial chunk.
	 */
	...
	/**
	 * Processes responses from downstream datanodes in the pipeline and sends back
	...
	class PacketResponder implements Runnable, Closeable {
		...
		/**
		 * Send an OOB response. If all acks have been sent already for the block and
		 * the responder is about to close, the delivery is not guaranteed. This is
		 * because the other end can close the connection independently. An OOB coming
		 * from downstream will be automatically relayed upstream by the responder. This
		 * method is used only by originating datanode.
		 *
		 * @param ackStatus
			...
			while (isRunning() && !lastPacketInBlock) {
				...
				try {
					...
					if (Thread.interrupted() || isInterrupted) {
						/*
						 * The receiver thread cancelled this thread. We could also check any other
						 * status updates from the receiver thread (e.g. if it is ok to write to
						 * replyOut). It is prudent to not send any more status back to the client
						 * because this datanode has a problem. The upstream datanode will detect that
						 * this datanode is bad, and rightly so.
						 *
						 * The receiver thread can also interrupt this thread for sending an out-of-band
						 * response upstream.
						 */

...
class QuorumCall<KEY, RESULT> {
  ...
  public synchronized void waitFor(
    ...
    while (true) {//多次尝试的意思
      ...
      //突然 Nmaenode 发生Full GC -》（如果连java的full gc都不知道的话，需要补一下知识）
      //如果你的垃圾比较好，这个full gc的过程就有可能持续几十秒（30秒），甚至几分钟。

      //代码 full GC -> 30s,工作线程是停止
      ...
      if (now > nextLogTime) {
        ...
        //如果等待的时间超过了    7 >  20*0.7 那么就打印日志，warn的日志
      ...
      /**
       * 第一次进来：计算还剩多少时间。16:00:20 - 16:00:01 = 19秒
       * 第二次进来：计算还剩多少时间。16:00:20 - 16:00:07 = 13秒
       *
       * 16:00:20 - 16:00:31
       * -11
       */
...
//             //就要判读是否发生了full gc
      ...
      stopWatch.restart();
      /**
       * 第一次进来：Math.min(19,16:00:06 -16:00:01) => 5
       * 第二次进来：Math.min(13,16:00:08 -16:00:06) => 2
       */
      rem = Math.min(rem, nextLogTime - now);
      /**
       * 第一次进来：Math.max(5,1) => 5
       * 第二次进来：Match.max(2,1) => 2
       */
      rem = Math.max(rem, 1);
      ...
      /**
       * 判断超时的时间5秒我把写死了，这是不应该，大家可以学习
       * 我们第一个bug的地方，自己修改一下，修改成能进行配置的
       */
    }
